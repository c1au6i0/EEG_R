...
for(x in file) {
    original <- read.csv( x , header = TRUE, sep = "," )
    ...
    eeg_date <- as.character(eeg$date[1])
    ...
    wdir <- getwd()
    msgBox(c("An heatmap of subj", as.character(eeg$subject[1]), " has been created in ",  wdir) )
    
    ###########
    # putt all the files togheter #
    ###########
    
    if (exists("alleeg")) {
          alleeg <- rbind( alleeg, eeg,row.names=NULL )
          maxtime <- append( maxtime, max(eeg$time_sec) )
          } else {
            alleeg <- eeg
            maxtime <- max(eeg$time_sec)
    }
}
